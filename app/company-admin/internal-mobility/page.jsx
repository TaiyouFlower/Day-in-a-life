...
const API_KEY = "AIzaSyDxikQo5DJowQG_fGAKD2cDucqA0DvAmbw";
let genAI;
let model;
...
try {
  if (API_KEY) {
    ...
    console.warn(
      "Gemini API Key not found. AI features will be disabled. Please set NEXT_PUBLIC_GEMINI_API_KEY in .env.local"
    );
  }
} catch (error) {
...
export default function InternalMobilityPage() {
  ...
  const fromRoles = useMemo(
    () => (fromDept ? rolesByDept[fromDept] || [] : []),
    [fromDept]
  );
  const toRoles = useMemo(
    () => (toDept ? rolesByDept[toDept] || [] : []),
    [toDept]
  );
  ...
  const validateInputs = () => {
    ...
    if (!fromDept || !fromRole || !toDept || !toRole) {
      setValidationError(
        "Please select 'From' and 'To' departments and roles."
      );
      return false;
    }
    if (fromExp === "" || isNaN(experience) || experience <= 0) {
      setValidationError(
        "Please enter a valid experience number greater than 0."
      );
      return false;
    }
    if (fromDept === toDept && fromRole === toRole) {
      setValidationError(
        "The 'From' and 'To' roles cannot be identical for analysis."
      );
      return false;
    }
    setValidationError("");
  ...
  const handleAnalyze = async () => {
    ...
    if (!model) {
      setAnalysisError(
        "AI analysis unavailable: Check API Key setup or console for errors."
      );
      return;
    }
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        console.error(
          "Could not find valid JSON object structure in response."
        );
        throw new Error(
          "AI response did not contain a recognizable JSON object."
        );
      }
      ...
      // Validate structure
      if (
        typeof parsedResult.time === "string" &&
        Array.isArray(parsedResult.strengths) &&
        Array.isArray(parsedResult.weaknesses) &&
        Array.isArray(parsedResult.benefits) &&
        Array.isArray(parsedResult.disadvantages)
      ) {
        setAnalysisResult(parsedResult);
      } else {
        console.error(
          "Parsed AI response missing required keys or has incorrect types:",
          parsedResult
        );
      ...
      setAnalysisError(
        `Failed to get or parse analysis from AI. Please check the console for the raw AI response. Error: ${error.message}`
      );
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-blue-900 flex flex-col items-center px-4 sm:px-6 md:px-8 py-4 overflow-y-auto text-white font-sans">
      {" "}
      ...
      <main className="w-full max-w-7xl mx-auto flex-1 mb-10">
        ...
        <section className="mb-8 bg-white/5 backdrop-blur-lg rounded-2xl border border-white/15 p-6 md:p-8 shadow-xl">
          <h2 className="text-xl md:text-2xl font-semibold text-white mb-6 border-b border-white/15 pb-4">
            Analyze Potential Transition
          </h2>
            ...
            <div className="space-y-5 bg-black/10 p-5 rounded-lg border border-white/10 shadow-inner">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2.5 bg-white/90 backdrop-blur-sm text-black rounded-md border-0 focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm shadow-sm"
                >
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!fromDept}
                  className="w-full p-2.5 bg-white/90 backdrop-blur-sm text-black rounded-md border-0 focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm disabled:bg-gray-500/30 disabled:cursor-not-allowed disabled:text-gray-400 shadow-sm"
                >
                  <option value="" className="text-gray-500">
                    Select Role...
                  </option>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white/90 backdrop-blur-sm text-black rounded-md border-0 focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm shadow-sm placeholder-gray-500"
                />
              </div>
            </div>
          ...
          {validationError && (
            <p className="text-sm text-red-400 mt-5 text-center font-medium animate-pulse">
              {validationError}
            </p>
          )}
          ...
          <div className="mt-8 text-center">
            ...
            >
              {isAnalyzing ? (
                ...
                "✨ Analyze Transition (AI)"
              )}
            </button>
            {!API_KEY && model !== null && (
              <p className="text-xs text-orange-400 mt-2">
                AI Analysis unavailable: API Key configuration error.
              </p>
            )}
            {model === null && (
              <p className="text-xs text-orange-400 mt-2">
                AI Analysis unavailable: API Key missing or invalid setup.
              </p>
            )}
          </div>
        </section>
        ...
        {analysisError && !isAnalyzing && (
          <div className="mt-8 p-5 bg-red-900/60 border border-red-500/50 text-red-200 rounded-lg animate-fade-in shadow-md backdrop-blur-sm">
            <h3 className="font-semibold mb-2 flex items-center text-red-100">
              <svg
            ...
            <p className="text-xs mt-2 opacity-80">
              Check the browser console for the raw AI response which might help
              diagnose the issue.
            </p>
          </div>
        )}
        ...
        {analysisResult && !isAnalyzing && !analysisError && (
          <section className="mt-8 animate-fade-in">
            <h2 className="text-xl font-semibold text-white mb-5 border-b border-white/15 pb-3">
              AI-Generated Transition Analysis
            </h2>
            {/* Results Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {/* Time Card */}
              <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/15 shadow-lg">
                {" "}
                <h3 className="font-semibold text-blue-400 mb-2 flex items-center text-sm">
                  <svg
                ...
                <p className="text-white text-base font-medium">
                  {analysisResult.time || "N/A"}
                </p>{" "}
              </div>
              {/* Strengths Card */}
              <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/15 shadow-lg">
                {" "}
                <h3 className="font-semibold text-green-400 mb-2 flex items-center text-sm">
                  <svg
                    className="w-4 h-4 mr-2 flex-shrink-0"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                ...
                <ul className="list-disc list-inside text-gray-300 text-xs space-y-1 pl-2">
              ...
              <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/15 shadow-lg">
                {" "}
                <h3 className="font-semibold text-red-400 mb-2 flex items-center text-sm">
                  <svg
                    className="w-4 h-4 mr-2 flex-shrink-0"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
              ...
              <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/15 shadow-lg">
                {" "}
                <h3 className="font-semibold text-purple-400 mb-2 flex items-center text-sm">
                  <svg
                    className="w-4 h-4 mr-2 flex-shrink-0"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
              ...
              {/* Disadvantages Card */}
              <div className="bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/15 shadow-lg md:col-span-2">
                {" "}
                <h3 className="font-semibold text-yellow-400 mb-2 flex items-center text-sm">
                  <svg
                    className="w-4 h-4 mr-2 flex-shrink-0"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                ...
                <ul className="list-disc list-inside text-gray-300 text-xs space-y-1 pl-2">
                  {(analysisResult.disadvantages || []).map((item, i) => (
                    <li key={`dis-${i}`}>{item}</li>
                  ))}{" "}
                  {!(analysisResult.disadvantages?.length > 0) && (
                    <li className="italic text-gray-500">None specified</li>
                  )}{" "}
                </ul>{" "}
              </div>
