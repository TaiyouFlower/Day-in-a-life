...
export default function TutorialBot() {
  ...
  const currentStep = simulationData.steps.find(
  ...
  if (currentStep.type === "waitForAppOpen") {
    ...
    console.log(
      `TutorialBot hidden: Waiting for user to open ${currentStep.app}`
    );
    return null;
  }
  ...
  return (
    ...
    <div className="absolute bottom-20 right-4 flex items-end gap-2 z-40 animate-fade-in">
      {" "}
      {/* Use fade-in, maybe remove bounce */}
      {/* Speech Bubble */}
      <div className="relative bg-white rounded-lg p-4 shadow-xl w-124">
